...
func UpdateUser(w http.ResponseWriter, r *http.Request) {
	...
	if userId != userIdToken {
		responses.Error(w, http.StatusForbidden, errors.New("It's not possible to update a user that is not yours"))
		return
	}
...
func UpdatePassword(w http.ResponseWriter, r *http.Request) {
	...
	if userIdToken != userId {
		responses.Error(w, http.StatusForbidden, errors.New("It's not possible to change the password of a user that is not your"))
		return
	}
	...
	repository := repositories.NewUserRepository(db)
	passwordOnDatabase, err := repository.SearchPassword(userId)
	if err != nil {
		responses.Error(w, http.StatusInternalServerError, err)
		return
	}

	if err = safety.CheckPassword(passwordOnDatabase, password.Current); err != nil {
		responses.Error(w, http.StatusUnauthorized, errors.New("The current password is not correct"))
		return
	}
